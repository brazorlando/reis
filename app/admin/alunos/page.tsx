...
export default function AlunoDetalhePage({
  ...
  const [abaAtiva, setAbaAtiva] = useState<Aba>("boletim");
  ...
  async function carregar() {
    ...
    // Buscar turma + classe + área + disciplinas
    ...
    // Buscar notas + faltas em paralelo
    const [notasRes, faltasRes] = await Promise.all([
      supabase.from("notas").select("*").eq("aluno_id", id),
      supabase.from("faltas").select("*").eq("aluno_id", id),
    ]);
  ...
  const estatisticasFaltas = useMemo(() => {
    const todas = faltas.filter(
      (f) => f.tipo !== "presenca"
    );
    const resumo = contarFaltas(faltas);
    return { ...resumo, registos: todas.length };
  }, [faltas]);
            ...
            <>
              ...
              {faltas.filter((f) => f.tipo !== "presenca").length === 0 ? (
                <EmptyState
                  ...
                  titulo="Sem faltas registadas"
                  descricao="Este aluno tem presença perfeita até agora."
                />
              ) : (
                <Table>
                  <TableHead>
                  ...
                  <TableBody>
                    {disciplinas.map((d) => {
                      ...
                      const contagem = contarFaltas(dFaltas);
                      ...
                      return (
                        <TableRow key={d.id}>
                          ...
                          <TableCell className="text-center">
                            {contagem.justificadas}
                          </TableCell>
                          <TableCell className="text-center">
                            {contagem.atrasos}
                          </TableCell>
                          <TableCell className="text-center">
                            <Badge variant={corFaltas(contagem.total)}>
                              {contagem.total} falta
                              {contagem.total !== 1 ? "s" : ""}
                            </Badge>
                          </TableCell>
                        </TableRow>
